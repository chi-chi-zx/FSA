...
class FSAProxySegmentation(BaseSegmentor):
    def __init__(self, clip_type, model_type, vfm_model, name_path, checkpoint=None, device=torch.device('cuda'),
                 prob_thd=0.0, logit_scale=40, beta=1.2, gamma=3.0, slide_stride=112, slide_crop=336, lamda=3.0, FSA=True):
        ...
        with torch.no_grad():
            for qw in query_words:
                ...
                feature = self.clip.encode_text(query) # 80x512
    ...
    def forward_feature(self, img, logit_size=None):
        ...
        image_features = self.clip.encode_image(img.half(), self.query_features,
